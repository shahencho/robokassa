...
async def add_user_to_groups(user_id: int):
    ...
    for group_id in GROUP_IDS:
        try:
            await application.bot.invite_chat_member(chat_id=group_id, user_id=user_id)
...
def mock_robokassa_response(user_id: int):
    ...
    asyncio.run(add_user_to_groups(user_id))
...
def check_subscriptions():
    ...
    for user_id in expired_users:
        asyncio.run(remove_user_from_groups(user_id))
...
async def start():
    ...
    mock_robokassa_response(test_user_id)
